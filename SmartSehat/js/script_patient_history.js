// JavaScript for patient_history.html
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Static patient history data
    const patientHistories = {
        'john-doe': {
            name: 'John Doe',
            history: [
                { title: 'Annual Checkup', date: '2023-01-15', medicines: 'Multivitamins', details: 'Routine checkup. All vitals normal.' },
                { title: 'Flu Treatment', date: '2023-03-10', medicines: 'Antiviral meds', details: 'Treated for seasonal flu.' }
            ]
        },
        'jane-smith': {
            name: 'Jane Smith',
            history: [
                { title: 'Dermatology Visit', date: '2023-02-20', medicines: 'Creams', details: 'Skin condition treatment.' },
                { title: 'Blood Test', date: '2023-04-05', medicines: 'Iron supplements', details: 'Anemia detected and treated.' }
            ]
        },
        'alex-johnson': {
            name: 'Alex Johnson',
            history: [
                { title: 'Orthopedic Consultation', date: '2023-05-12', medicines: 'Pain relievers', details: 'Knee pain assessment.' }
            ]
        }
    };
    ...
    patientSelect.addEventListener('change', (e) => {
        ...
        if (patientId && patientHistories[patientId]) {
            const patient = patientHistories[patientId];
            patientName.textContent = patient.name;
            // Combine static history and uploaded reports
            const combinedHistory = [...patient.history, ...historyData.filter(item => item.details.startsWith('Uploaded report:'))];
    ...
    function renderHistory(history) {
        ...
        history.forEach(item => {
            ...
            itemDiv.innerHTML = `
                ...
                <p><strong>Medicines:</strong> ${item.medicines || 'N/A'}</p>
